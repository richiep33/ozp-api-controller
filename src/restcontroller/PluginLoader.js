 ...
 */
var PluginLoader = function(middleware, security, pluginOptions, apiOptions, requestFn) {
...
PluginLoader.prototype.assignRoute = function(routeObject) {
    try {
        ...
        routeObject.middleware[httpMethod](routeObject.uri, this.security.authenticate('basic'), routeObject.requestFn);
...
PluginLoader.prototype.parsePluginLocation = function(error, files) {
    ...
    for (fileIter; fileIter < files.length; fileIter++) {
        ...
        if (file.match(/^ozp-.*/) || file.match(/^aml-.*/)) {
            pluginFolders.push(file);
        }
    }
